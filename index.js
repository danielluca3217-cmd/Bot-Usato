...
async function adminAdd(opts, interaction, username) {
    ...
    if (await aggiungiAdminWhitelist(utenteTarget.id, utenteTarget.username, username)) {
        const embed = new EmbedBuilder()
            .setColor(0x00ff00)
            .setTitle('✅ Admin del sito aggiunto!')
            .setDescription(`👤 **${utenteTarget.username}** può ora accedere al pannello admin del sito.`)
            .addFields(
                { name: '🆔 Discord ID', value: utenteTarget.id, inline: true }
            )
            .setFooter({ text: `Aggiunto da: ${username}` })
            .setTimestamp();
        return { success: true, embed };
    }
    return { success: false, messaggio: '❌ Errore durante il salvataggio.' };
...
async function adminRemove(opts, interaction, username) {
    ...
    if (await rimuoviAdminWhitelist(utenteTarget.id)) {
        const embed = new EmbedBuilder()
            .setColor(0xff0000)
            .setTitle('🗑️ Admin del sito rimosso!')
            .setDescription(`👤 **${utenteTarget.username}** non può più accedere al pannello admin del sito.`)
            .setFooter({ text: `Rimosso da: ${username}` })
            .setTimestamp();
        return { success: true, embed };
    }
...
async function adminList() {
    ...
    let testo = '📋 **Admin autorizzati sul pannello del sito:**\n\n';
    whitelist.forEach(w => {
        testo += `• **${w.discord_username || 'sconosciuto'}** (ID: ${w.discord_id}) — aggiunto da ${w.aggiunto_da || '?'} il ${w.aggiunto_il}\n`;
    });
    const embed = new EmbedBuilder()
        .setColor(0x0099ff)
        .setTitle('📋 Whitelist admin sito')
        .setDescription(testo)
        .setFooter({ text: `Totale: ${whitelist.length} admin` })
        .setTimestamp();
    return { success: true, embed };
}
